...
Команда `hnh` будет доступна в этом окружении. Астрологический слой (натал/транзиты) входит в ядро; для точного расчёта позиций планет нужна папка **ephe** в корне репозитория с файлами эфемерид (.se1). Проверка: `hnh.astrology.ephemeris.check_ephe_available()`. Без файлов Swiss Ephemeris использует встроенный метод (менее точный).
