...
- **`.github/workflows/ci.yml`**: при каждом push и при каждом PR в `main` запускаются тесты (pytest).
- **`.github/workflows/release.yml`**: при пуше тега `v*` (например `v0.1.0`) запускаются тесты, сборка пакета (sdist + wheel), создаётся GitHub Release с прикреплёнными артефактами. Тег создаётся вручную: `git tag v0.1.0 && git push origin v0.1.0`.
