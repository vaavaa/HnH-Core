...
**Constraints**: Обратная совместимость replay_v2 и run_step_v2; не более 2–3 уровней наследования; без магических флагов внутри логики
...
- [ ] **Existing API**: replay_v2, extract_replay_config, assembler, modulation остаются рабочими; новые классы — фасад/оркестрация.
...
- **NatalChart**: конструктор от birth_data (дата/время/место или готовые позиции); внутренне вызывает существующую логику ephemeris/houses; хранит список Planet и список Aspect; метод `compute_base_energy()` или эквивалент для передачи в BehavioralCore. Инвариант: после построения объект immutable для replay.
...
- **ZodiacExpression**: конструктор принимает NatalChart (или данные для построения view); вычисляет sign_vectors, dominant_sign, dominant_element по логике 004; read-only, не мутирует натал.
...
- **BehavioralCore**: конструктор `BehavioralCore(natal, identity_config)`. identity_config — Identity (IdentityCore) или конфиг, дающий base_vector и sensitivity_vector; sensitivity_vector живёт в Identity, не дублируется в BehavioralCore. Метод `apply_transits(transit_state: TransitState)` принимает выход TransitEngine; использует `transit_state.bounded_delta` и sensitivity из identity; делегирует в state.assembler (границы уже в TransitState). Не содержит fatigue/will/death.
...
- **Agent**: конструктор `Agent(birth_data, config=None, lifecycle: bool = False)`; при необходимости принимает identity_config (или строит Identity из натала).
  - Создаёт `self.natal = NatalChart(birth_data)`.
  - Создаёт identity_config (IdentityCore или из натала/конфига), затем `self.behavior = BehavioralCore(self.natal, identity_config)`.
  ...
  - Метод `step(date)` — порядок: (1) transit_state = self.transits.state(date, config); (2) при self.lifecycle — update_lifecycle(transit_state.stress, resilience) — lifecycle видит поведение прошлого шага; (3) self.behavior.apply_transits(transit_state). Resilience из base_vector (Stability axis) по 005.
...
- Не удалять и не ломать вызовы `replay_v2`, `run_step_v2`, `extract_replay_config`, `assemble_state`, `apply_bounds`, логику в modulation/delta. Новый сценарий использования — через Agent; старый — по-прежнему через прямые вызовы модулей.
- При необходимости в документации указать: «рекомендуемый путь — Agent; низкоуровневый API остаётся для скриптов и тестов».
...
- **Регрессия replay**: интеграционные тесты — два прогона (replay_v2 и Agent.step по тем же датам) с сравнением params_final / axis_final и при lifecycle — F, W, state.
